...
function FacultyPasswordChangePage({
  onBack,
  onSuccess,
  onNavigate,
  onOpenStudentForm,
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/faculty-accounts/change-password/`, {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        throw new Error(data.detail || "Unable to update faculty password.");
      }

      onSuccess(data);
    } catch (error) {
      setErrorMessage(error.message || "Unable to update faculty password.");
    } finally {
...
export default FacultyPasswordChangePage;
